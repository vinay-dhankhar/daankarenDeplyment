...
const CampaignPage = ({ role }) => {
  ...
  useEffect(() => {
    const fetchCampaign = async () => {
      try {
        const response = await fetch(`https://daankaren-deplyment-server.vercel.app/campaigns/${campaignId}`);
        if (!response.ok) {
  ...
  const handleVerify = async () => {
    try {
      const response = await fetch(`https://daankaren-deplyment-server.vercel.app/campaigns/${campaignId}/approve`, {
        method: 'POST',
      });
